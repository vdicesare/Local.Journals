...
##### KNOWLEDGE BRIDGING COMMUNICATION FUNCTION
### references
...
### languages
# MJL language data upload and mining (mainstream = English)
...
# create a new variable to identify if a journal publishes in mainstream language (= English in MJL, ENG in Scopus, English in DOAJ and en in OpenAlex)
openalex_journals <- openalex_journals %>% mutate(mainstream_lang = case_when(!is.na(scopus_lang) & str_detect(scopus_lang, "\\bENG\\b") ~ 1,
                                                                              !is.na(scopus_lang) ~ 0,
                                                                              !is.na(mjl_lang) & (str_detect(mjl_lang, "English")) ~ 1,
                                                                              !is.na(mjl_lang) ~ 0,
...
### clean names and remove duplicated variables
openalex_journals <- openalex_journals %>% select(-journal_name.y, -journal_name) %>%
                                           rename(journal_name = journal_name.x) %>%
                                           rename(topic = topic_display_name)

knowledge_bridging_journals <- knowledge_bridging_journals %>% select(-journal_name.y, -journal_name) %>%
                                                               rename(journal_name = journal_name.x) %>%
                                                               rename(topic = topic_display_name)
